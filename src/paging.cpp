...
namespace geodis
{
	...
	struct page_index_comparator : public std::binary_function<page_index_pointer, page_index_pointer, bool>
	{
	...
	static std::unordered_map<page_index_pointer, page*, page_index_hasher, page_index_comparator> _pageIndex;

	static boost::mutex _indexMutex;
	...
	void registry::initPagingSystem(int maxPagesInMemory)
	{
		...
		for (int i = 0; i < maxPagesInMemory; i++) _pagePoolMapper[i] = i;
